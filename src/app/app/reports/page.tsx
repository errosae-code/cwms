...
export default async function Reports({ searchParams }: { searchParams: Promise<{ year?: string }> }) {
  ...
  const displayEnd = year === now.getFullYear()
    ...
    : `31 December ${year}`;
  const displayPeriod = year === now.getFullYear()
    ? `January to ${now.toLocaleDateString("en-KE", { month: "long" })} ${year}`
  ...
  const currentMonthNumber = year === now.getFullYear() ? now.getMonth() + 1 : 12;

  return <>
    <div className="pagehead"><div><h1>Financial Statement</h1><p className="muted">Statement from 1 January through {displayEnd}, with consolidated totals for every member.</p></div></div>
    ...
    <div className="statement-title">
      <h2>Statement — {displayPeriod}</h2>
      <div>{settingsRes.data?.report_title || "Chyun Welfare Statement"}</div>
    </div>
    ...
    <div className="grid2">
      <div className="panel"><h3>Money In</h3><table><tbody>
      ...
      <div className="panel"><h3>Money Out</h3><table><tbody>
    ...
    <div className="panel" style={{ marginTop: 18 }}><h3>Fund Position — {displayEnd}</h3><div className="summarygrid">
    ...
    <div className="panel contribution-status-panel" style={{ marginTop: 18 }}>
      <div className="contribution-status-head"><div><h3>Member Summary</h3><span>1 January to {displayEnd} — consolidated totals per member</span></div></div>
      <div className="tablewrap contribution-status-wrap"><table className="contribution-status-table"><thead><tr>
        ...
        {memberRows.map((m: any) => {
          ...
          const activeMonths = joined.getFullYear() < year ? 12 : joined.getFullYear() > year ? 0 : Math.max(0, Math.min(currentMonthNumber, 12) - (joined.getMonth() + 1) + 1);
    ...
    <div className="panel transactions-panel" style={{ marginTop: 18 }}><h3>Transactions — {displayPeriod}</h3><div className="reportsections">
