...
export default async function Reports({ searchParams }: { searchParams: Promise<{ month?: string; year?: string; view?: string }> }) {
  ...
  const [settingsRes, allBeforeContrib, allBeforeLoans, allBeforeReps, allBeforeWelfare, allBeforeRefunds, monthContrib, monthLoans, monthReps, monthWelfare, monthRefunds, membersRes, scheduleContrib, loansToEnd, repsToEnd, joinedMonth, allRefundsToEnd] = await Promise.all([
  ...
  const beforeRefunds = allBeforeRefunds.data?.reduce((a,x)=>a+num(x.amount),0) || 0;
  ...
  const refundsThisMonth = monthRefunds.data?.reduce((a,x)=>a+num(x.amount),0) || 0;
  ...
  const expected = num(settingsRes.data?.monthly_contribution) || 500;
  const memberNo = (v: unknown) => Number(String(v ?? "").replace(/\D/g, "")) || Number.MAX_SAFE_INTEGER;
  const eligibleMembers = (membersRes.data || []).sort((a:any,b:any)=>memberNo(a.membership_no)-memberNo(b.membership_no));
  ...
  <div className="card">
    ...
    <div className="panel" style={{marginTop:18}}><h3>Member Contribution Status — {monthNames[month-1]}</h3><div className="tablewrap"><table><thead><tr><th>Member</th><th>Status</th><th>Expected</th><th>Paid</th><th>Variance</th><th>Payment Status</th></tr></thead><tbody>{eligibleMembers.map((m:any)=>{
      ...
      const withdrawnBeforeMonth=m.status==="withdrawn"&&refund?new Date(`${refund.refund_date}T00:00:00`)<selectedMonthStart:false;
      const due = !isFutureMonth && !joinedAfterMonth && !withdrawnBeforeMonth;
      const memberExpected = due ? expected : 0;
