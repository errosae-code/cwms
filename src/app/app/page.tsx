...
export default async function Home(){const s=await createClient();
 const [c,m,l,w,settings,reps,refundRes,membersReg]=await Promise.all([
  ...
  s.from("loans").select("principal,outstanding_principal,outstanding_interest,status,due_date").is("deleted_at",null),
 ...
 const queryErrors=[c,m,l,w,settings,reps,refundRes,membersReg].filter(r=>r.error);
 if(queryErrors.length>0)return <><h1>Home</h1><div className="notice">Unable to load the latest financial data. Please refresh or sign in again.</div></>;
 ...
 const today=kenyaToday();const activeLoans=l.data?.filter(x=>Number(x.outstanding_principal||0)+Number(x.outstanding_interest||0)>0).length||0;const overdue=l.data?.filter(x=>Number(x.outstanding_principal||0)+Number(x.outstanding_interest||0)>0&&String(x.due_date||"")<today).length||0;
