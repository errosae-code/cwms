...
export default async function Repayments(){
  ...
  </div>{!row&&<p className="notice" style={{marginTop:14,marginBottom:0}}>A receipt number will be generated automatically and the printable receipt will open after saving.</p>}<button className="btn success" style={{marginTop:14}}>{row?"Save Changes":"Save Repayment"}</button></form>;
  return <>
    <div className="pagehead"><div><h1>Repayments</h1><p className="muted">Repayments are allocated to outstanding interest first, then principal. Receipts are generated automatically.</p></div><Modal trigger="+ Add Repayment" title="Record Repayment">{form(recordRepayment)}</Modal></div>
    <div className="panel"><div className="tablewrap"><table><thead><tr><th>Loan</th><th>Member</th><th>Amount</th><th>Interest</th><th>Principal</th><th>Date</th><th>Receipt</th><th>Actions</th></tr></thead><tbody>{rows?.length?rows.map((r:any)=><tr key={r.id}><td>{r.loans?.loan_no}</td><td>{r.loans?.members?.membership_no} — {r.loans?.members?.full_name}</td><td>KES {Number(r.amount).toLocaleString()}</td><td>KES {Number(r.interest_paid).toLocaleString()}</td><td>KES {Number(r.principal_paid).toLocaleString()}</td><td>{r.payment_date}</td><td><Link className="textlink" href={`/app/repayments/${r.id}/receipt`}>{r.receipt_no||"View Receipt"}</Link></td><td><div className="rowactions"><Link className="btn light" href={`/app/repayments/${r.id}/receipt`}>Receipt</Link><Modal trigger="Edit" title="Edit Repayment">{form(updateRepayment,r)}</Modal><form action={deleteRepayment}><input type="hidden" name="id" value={r.id}/><ConfirmSubmitButton message="Delete this repayment? Loan balances will be recalculated and the action logged."/></form></div></td></tr>):<tr><td colSpan={8} className="empty">No repayments recorded.</td></tr>}</tbody></table></div></div>
  </>;
}
