...
import OverdueReminderButtons from "@/components/OverdueReminderButtons";
...
export default async function Loans({searchParams}:{searchParams:Promise<{saved?:string}>}){
  const sp=await searchParams;
  const s=await createClient();
  const [{data:memberData},{data:rows}]=await Promise.all([
    s.from("members").select("id,membership_no,full_name,phone").eq("status","active").is("deleted_at",null),
    s.from("loans").select("*,members(full_name,membership_no,phone)").is("deleted_at",null).order("issue_date",{ascending:false}),
  ]);
  ...
  </div><button className="btn success" style={{marginTop:14}} type="submit">{row?"Save Changes":"Issue Loan"}</button></form>;
  return <>
    {sp.saved==="1"&&<div role="status" aria-live="polite" style={{marginBottom:16,padding:"12px 16px",borderRadius:10,background:"#ECFDF3",border:"1px solid #ABEFC6",color:"#067647",fontWeight:700}}>✓ Saved successfully. Your loan information has been updated.</div>}
    ...
    <div className="panel"><div className="tablewrap"><table><thead><tr><th>Loan</th><th>Member</th><th>Issue Date</th><th>Due Date</th><th>Principal Outstanding</th><th>Interest Outstanding</th><th>Total Outstanding</th><th>Status</th><th>Actions</th></tr></thead><tbody>{rows?.length?rows.map((r:any)=>{const outstanding=Number(r.outstanding_principal||0)+Number(r.outstanding_interest||0);const storedStatus=r.status;const liveStatus=outstanding<=0?"cleared":String(r.due_date||"")<today?"overdue":"active";const delta=daysFromToday(String(r.due_date||today));const dueInfo=liveStatus==="cleared"?"":liveStatus==="overdue"?`${Math.abs(delta)} day${Math.abs(delta)===1?"":"s"} overdue`:delta===0?"Due today":`${delta} day${delta===1?"":"s"} remaining`;return <tr key={r.id}><td>{r.loan_no}</td><td>{r.members?.membership_no} — {r.members?.full_name}</td><td>{dateText(r.issue_date)}</td><td><strong className="due-date">{dateText(r.due_date)}</strong>{dueInfo&&<small className="dueinfo">{dueInfo}</small>}</td><td>KES {Number(r.outstanding_principal).toLocaleString()}</td><td>KES {Number(r.outstanding_interest).toLocaleString()}</td><td>KES {outstanding.toLocaleString()}</td><td><span className={`badge ${liveStatus}`}>{liveStatus}</span>{storedStatus!==liveStatus&&<small className="dueinfo">Live status</small>}</td><td><div className="rowactions"><Modal trigger="Edit" title={`Edit ${r.loan_no}`}>{form(updateLoan,r)}</Modal>{liveStatus==="overdue"&&<OverdueReminderButtons name={r.members?.full_name||"Member"} phone={r.members?.phone||""} loanNo={r.loan_no} dueDate={dateText(r.due_date)} daysOverdue={Math.abs(delta)} principal={Number(r.outstanding_principal||0)} interest={Number(r.outstanding_interest||0)} total={outstanding}/>}<form action={deleteLoan}><input type="hidden" name="id" value={r.id}/><ConfirmSubmitButton message="Delete this loan? Loans with repayments cannot be deleted."/></form></div></td></tr>}):<tr><td colSpan={9} className="empty">No loans recorded.</td></tr>}</tbody></table></div></div>
  </>;
}
